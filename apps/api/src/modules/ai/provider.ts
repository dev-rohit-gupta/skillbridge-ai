...
export interface AiProvider {
  ...
  extractJobRequirements(input: JobExtractionInput): Promise<ExtractedRequirement[]>;
}
...
function requirementContext(text: string, index: number) {
  const previousBoundaries = [text.lastIndexOf(".", index - 1), text.lastIndexOf("\n", index - 1), text.lastIndexOf(";", index - 1)];
  const start = Math.max(0, Math.max(...previousBoundaries) + 1);
  const nextBoundaries = [text.indexOf(".", index), text.indexOf("\n", index), text.indexOf(";", index)].filter((value) => value >= 0);
  const end = nextBoundaries.length ? Math.min(...nextBoundaries) + 1 : Math.min(text.length, index + 260);
  return text.slice(start, end).replace(/\s+/g, " ").trim();
}

function classifyImportance(context: string) {
  const value = context.toLowerCase();
  if (/preferred|nice to have|good to have|bonus|familiarity with|optional/.test(value)) {
    return {importance: "OPTIONAL" as const, weight: 1};
  }
  if (/must have|required|mandatory|strong knowledge|proficien|hands-on|minimum .*experience/.test(value)) {
...
export class DeterministicAiProvider implements AiProvider {
  ...
  async extractJobRequirements(input: JobExtractionInput): Promise<ExtractedRequirement[]> {
    const normalizedDescription = normalize(input.description);
    const found = new Map<string, ExtractedRequirement>();

    const terms = input.skills.flatMap((skill) => [skill.name, skill.slug, ...skill.aliases]
      .filter(Boolean)
    ...
    for (const entry of terms) {
      ...
      const pattern = new RegExp(`(^|[^a-z0-9+#])${escapeRegex(entry.normalizedTerm).replace(/\\ /g, "\\s+")}([^a-z0-9+#]|$)`, "i");
      const match = pattern.exec(normalizedDescription);
      if (!match) continue;
      const rawIndex = input.description.toLowerCase().indexOf(entry.term.toLowerCase());
