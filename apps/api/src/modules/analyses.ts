...
import {createAnalysisInputSchema, updateRoadmapItemStatusInputSchema} from "@skillbridge/shared";
import {db} from "../config/database";
import {AppError} from "../lib/errors";
import {authenticate} from "../middleware/auth";
import {validateBody} from "../middleware/validate";
import {scoreAnalysis, type ScoringRequirement} from "./scoring";

async function roleTarget(roleId: string) {
  const [role] = await db.select().from(careerRoles).where(and(
    eq(careerRoles.id, roleId),
    eq(careerRoles.isActive, true),
  )).limit(1);
  if (!role) throw new AppError(404, "CAREER_ROLE_NOT_FOUND", "Career role not found.");
  ...
  const rows = await db.select({
    ...
    .leftJoin(roleRequirementSkills, eq(roleRequirementSkills.roleRequirementId, roleRequirements.id))
    .where(and(eq(roleRequirements.careerRoleId, role.id), eq(roleRequirements.isActive, true)))
    .orderBy(asc(roleRequirements.sortOrder));

  const requirements = Object.values(rows.reduce<Record<string, ScoringRequirement>>((accumulator, row) => {
    accumulator[row.id] ??= {
      id: row.id,
      name: row.name,
  ...
  return {
    target: {type: "PREDEFINED_ROLE" as const, id: role.id, name: role.name, version: role.version},
    requirements,
...
async function customJobTarget(userId: string, jobDescriptionId: string) {
  ...
  if (job.status !== "READY" || job.contentRevision !== job.confirmedRevision) {
    throw new AppError(409, "JOB_REQUIREMENTS_NOT_CONFIRMED", "Review and confirm the job requirements first.");
  }
  ...
  if (requirements.length < 3) throw new AppError(409, "ANALYSIS_TARGET_INVALID", "The job description needs at least three requirements.");

  return {
    target: {type: "CUSTOM_JOB" as const, id: job.id, name: job.title, companyName: job.companyName},
    requirements,
    careerRoleId: null,
...
analysisRouter.post("/", validateBody(createAnalysisInputSchema), async (req, res, next) => {
  try {
    ...
    if (resume.status !== "PROCESSED" || resume.contentRevision !== resume.confirmedRevision) {
      throw new AppError(409, "RESUME_NOT_CONFIRMED", "Review and confirm the resume before analysis.");
    }

    const selected = input.type === "PREDEFINED_ROLE"
      ? await roleTarget(input.careerRoleId)
      : await customJobTarget(req.auth!.userId, input.jobDescriptionId);
    const [profile] = await db.select().from(userProfiles)
      .where(eq(userProfiles.userId, req.auth!.userId)).limit(1);
    ...
    try {
      ...
      const skillIds = [...new Set(scored.requirements.flatMap((item) => [
      ...
      const skillRows = skillIds.length
        ...
        : [];
      const skillMap = new Map(skillRows.map((skill) => [skill.id, skill.name]));
      const result = {
        ...scored,
        ...
        requirements: scored.requirements.map((item) => ({
          ...item,
          acceptedSkills: item.acceptedSkillIds.map((id) => ({id, name: skillMap.get(id) ?? "Unknown"})),
          matchedSkill: item.matchedSkillId
            ? {id: item.matchedSkillId, name: skillMap.get(item.matchedSkillId) ?? "Unknown"}
            : null,
        })),
      };
      ...
      res.status(201).json({success: true, data: {id: analysis.id, ...result}});
    } catch (processingError) {
      await db.update(analyses).set({
        status: "FAILED",
        failureReason: processingError instanceof Error ? processingError.message : "Analysis failed.",
        updatedAt: new Date(),
      }).where(eq(analyses.id, analysis.id));
...
analysisRouter.get("/:analysisId", async (req, res, next) => {
  try {
    const [data] = await db.select().from(analyses).where(and(
      eq(analyses.id, req.params.analysisId as string),
      eq(analyses.userId, req.auth!.userId),
    )).limit(1);
    if (!data) throw new AppError(404, "ANALYSIS_NOT_FOUND", "Analysis not found.");
    res.json({success: true, data});
  } catch (error) { next(error); }
});
...
roadmapRouter.post("/analyses/:analysisId/roadmap", async (req, res, next) => {
  try {
    const [analysis] = await db.select().from(analyses).where(and(
      eq(analyses.id, req.params.analysisId as string),
      eq(analyses.userId, req.auth!.userId),
    )).limit(1);
    if (!analysis || analysis.status !== "COMPLETED" || !analysis.result) {
      throw new AppError(409, "ANALYSIS_NOT_READY", "Complete the analysis first.");
    }
    const [existing] = await db.select().from(roadmaps).where(eq(roadmaps.analysisId, analysis.id)).limit(1);
    ...
    const missing = (result.requirements ?? []).filter((item) => item.effectiveBp < 8000).slice(0, 8);
    const roadmap = await db.transaction(async (tx) => {
      const [created] = await tx.insert(roadmaps).values({
        userId: req.auth!.userId,
      ...
      if (missing.length) {
        await tx.insert(roadmapItems).values(missing.map((item, index) => ({
          ...
          priority: (item.importance === "CORE" ? "LEARN_FIRST" : item.importance === "IMPORTANT" ? "HIGH" : "OPTIONAL") as "LEARN_FIRST" | "HIGH" | "OPTIONAL",
...
roadmapRouter.get("/roadmaps/:roadmapId", async (req, res, next) => {
  try {
    const [roadmap] = await db.select().from(roadmaps).where(and(
    ...
    const items = await db.select().from(roadmapItems)
      .where(eq(roadmapItems.roadmapId, roadmap.id))
      .orderBy(asc(roadmapItems.sortOrder));
    const completed = items.filter((item) => item.status === "COMPLETED").length;
    res.json({
      success: true,
      data: {...roadmap, items, completionPercentage: items.length ? Math.round(completed / items.length * 100) : 0},
    });
  } catch (error) { next(error); }
});
...
roadmapRouter.get("/analyses/:analysisId/roadmap", async (req, res, next) => {
  try {
    const [data] = await db.select().from(roadmaps).where(and(
...
roadmapRouter.patch("/roadmaps/:roadmapId/items/:itemId/status", validateBody(updateRoadmapItemStatusInputSchema), async (req, res, next) => {
  try {
    const [roadmap] = await db.select().from(roadmaps).where(and(
      eq(roadmaps.id, req.params.roadmapId as string),
      eq(roadmaps.userId, req.auth!.userId),
    ...
    const [data] = await db.update(roadmapItems).set({
      status: input.status,
    ...
    if (!data) throw new AppError(404, "ROADMAP_ITEM_NOT_FOUND", "Roadmap item not found.");
    res.json({success: true, data});
  } catch (error) { next(error); }
});
